...
//
// Steps:
//   1. Build root tree from index via tree_from_index
//   2. Read current HEAD as parent (may not exist for first commit)
//   3. Fill Commit struct with author, timestamp, tree, parent, message
//   4. Serialize with commit_serialize
//   5. Write to object store with object_write(OBJ_COMMIT)
//   6. Update HEAD/branch ref with head_update
//
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 2: Read parent commit (first commit has no parent)
    ...
    // Step 6: Write serialized commit to the object store
