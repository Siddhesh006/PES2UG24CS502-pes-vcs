...
extern int object_write(ObjectType type, const void *data, size_t len,
                        ObjectID *id_out);
...
int tree_serialize(const Tree *tree, void **data_out, size_t *len_out) {
  ...
  for (int i = 0; i < sorted_tree.count; i++) {
    ...
    int written =
        sprintf((char *)buffer + offset, "%o %s", entry->mode, entry->name);
    offset += written + 1;
...
static int write_tree_level(const IndexEntry *entries, int count,
                            ObjectID *id_out, size_t prefix_len) {
  Tree tree;
  tree.count = 0;
  ...
  while (i < count) {
    ...
    if (!slash) {
      ...
      while (j < count) {
        ...
        if (dlen_j != dir_name_len ||
            strncmp(rel_j, dir_name, dir_name_len) != 0)
          break;
        j++;
      }

      size_t new_prefix = prefix_len + dir_name_len + 1;
      ObjectID subtree_id;
      if (write_tree_level(entries + i, j - i, &subtree_id, new_prefix) != 0)
        return -1;
...
int tree_from_index(ObjectID *id_out) {
  ...
  return write_tree_level(index.entries, index.count, id_out, 0);
}